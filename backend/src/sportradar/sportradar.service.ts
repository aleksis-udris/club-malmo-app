...
const A3: Record<string, string> = {
  SWE: 'SE', ENG: 'GB', GBR: 'GB', GER: 'DE', DEU: 'DE', FRA: 'FR', ESP: 'ES',
  NED: 'NL', NLD: 'NL', SUI: 'CH', CHE: 'CH', BEL: 'BE', ITA: 'IT', AUT: 'AT',
  WAL: 'GB', SCO: 'GB', FIN: 'FI', NOR: 'NO', DEN: 'DK', DNK: 'DK', POL: 'PL',
  EGY: 'EG', USA: 'US', IND: 'IN', MAS: 'MY', HKG: 'HK', QAT: 'QA', NZL: 'NZ',
}
...
export class SportradarService {
  ...
  async resolveSeasonId(): Promise<string> {
    ...
    if (this.cachedSeasonId)
      this.log.log(`Auto-selected current season: ${chosen?.name ?? '?'} (${this.cachedSeasonId})`)
  ...
  private toMatch(e: SrEvent) {
    ...
    return {
      ...
      home: country(e.homeName ?? comp?.[0]?.name ?? 'Home', comp?.[0]?.country_code),
      away: country(e.awayName ?? comp?.[1]?.name ?? 'Away', comp?.[1]?.country_code),
      score,
  ...
  async getLatest(_bracket: 'top' | 'bottom') {
    const rows = await this.events.find({ where: { status: 'closed' }, order: { scheduled: 'DESC' }, take: 6 })
  ...
  async getSweden() {
    ...
    const swedes = all.filter(
      (c) => c.country === 'Sweden' || (c.countryCode ?? '').toUpperCase().startsWith('SWE'),
    )
    const men = swedes.filter((c) => c.gender !== 'female').map((c) => ({ id: c.id, name: c.name, played: c.played }))
    const women = swedes.filter((c) => c.gender === 'female').map((c) => ({ id: c.id, name: c.name, played: c.played }))
    const groupStandings = (await this.standings.find({ order: { rank: 'ASC' } }))
      .filter((s) => (s.countryCode ?? '').toUpperCase().startsWith('SWE'))
      .map((s) => this.toStanding(s))
    return { groupStandings, overallStats: [], men, women, counts: { men: men.length, women: women.length } }
  }
